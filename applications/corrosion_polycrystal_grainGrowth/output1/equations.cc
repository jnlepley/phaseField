...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    for (int var_index=0; var_index<N; var_index++){
        ...
        set_dependencies_value_term_RHS(var_index, "n0, n1, n2, n3, n4, n5, grad(psi), cM, Phi");
        set_dependencies_gradient_term_RHS(var_index, "n0, n1, n2, n3, n4, n5, psi, cM, Phi, grad(mu0), grad(mu1), grad(mu2), grad(mu3), grad(mu4), grad(mu5)");
        
    }

    // Variable index: N
    set_variable_name				(N,"psi");
    set_variable_type				(N,SCALAR);
    set_variable_equation_type		(N,EXPLICIT_TIME_DEPENDENT);

    set_dependencies_value_term_RHS(N, "n0, n1, n2, n3, n4, n5, psi, grad(psi), cM, Phi");
    set_dependencies_gradient_term_RHS(N, "n0, n1, n2, n3, n4, n5, psi, cM, Phi, grad(mupsi)");
...
void customPDE<dim,degree>::explicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
    ...
    for (unsigned int i=0; i<N; i++){
        n[i] = variable_list.get_scalar_value(i);
        nx[i] = variable_list.get_scalar_gradient(i);
    ...
    scalarvalueType psi = variable_list.get_scalar_value(N);
    scalargradType psix = variable_list.get_scalar_gradient(N);
    ...
    for (unsigned int i=0; i<N; i++){
        ...
        variable_list.set_scalar_value_term_RHS(i,rnV[i]);
        variable_list.set_scalar_gradient_term_RHS(i,rnxV[i]);
    ...
    variable_list.set_scalar_value_term_RHS(N,rpsiV);
    variable_list.set_scalar_gradient_term_RHS(N,rpsixV);
